...
public class LocalFileHandler {
    ...
    public void createFile(String filePath, byte[] data, int start) {
        try {
            ...
            cache(new FileEvent(EventType.FILE_CREATE, filePath));
        } catch (IOException e) {
        }
    }
    ...
    public void modifyFile(String filePath, long blockIndex, long blockSize, byte[] data, int start) {
        try {
            ...
            cache(new FileEvent(EventType.FILE_MODIFY, filePath));
        } catch (IOException e) {
        }
    }
    ...
    public void deleteFile(String filePath) {
        ...
        cache(new FileEvent(EventType.FILE_DELETE, filePath));
    }

    public void createDirectory(String filePath) {
        new File(basePath, filePath).mkdirs();
        cache(new FileEvent(EventType.DIRECTORY_CREATE, filePath));
    }

    public void deleteDirectory(String filePath) {
        FileUtils.deleteQuietly(new File(basePath, filePath));
        cache(new FileEvent(EventType.DIRECTORY_DELETE, filePath));
    }
